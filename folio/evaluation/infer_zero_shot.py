...
# -*- coding: utf-8 -*-
"""
Direct Answer baseline for FOLIO - No CoT, No steering
...
# Direct answer prompt - 不要求 CoT，直接回答
DIRECT_PROMPT_TEMPLATE = """Given the premises, determine if the hypothesis is True, False, or Uncertain.
Output ONLY one word: True, False, or Uncertain.
...
def build_direct_prompt(prem: str, concl: str) -> str:
    return DIRECT_PROMPT_TEMPLATE.format(prem_nl=prem.strip(), hypo_nl=concl.strip())
...
def parse_direct_answer(text: str):
    ...
    for word in response.split():
        ...
        if word_clean in {"true", "t"}:
            return "True"
        if word_clean in {"false", "f"}:
            return "False"
        if word_clean in {"unknown", "uncertain", "u"}:
            return "Unknown"
...
def run_zero_shot_eval(mdl, tok, device, samples, max_new_tokens, batch_size):
    """
    运行 zero-shot 评测（无 steering）。返回结果字典。
    """
    results = []
    correct = 0
    parse_failed = 0
    ...
    for batch in tqdm(batched(samples, bs), total=(len(samples) + bs - 1) // bs, desc="Evaluating"):
        prompts = []
        for s in batch:
            ptxt = build_direct_prompt(s["prem"], s["concl"])
        ...
        for text, s in zip(texts, batch):
            ...
            results.append({
                "gold": gold, "pred": pred,
                "prem": s["prem"], "hypo": s["concl"], "gen": text
            })
...
def main():
    ...
    ap.add_argument("--out_csv", type=str, default="zero_shot_results.csv")
    ...
    # 输出结果
    print(f"\n{'='*60}")
    print("Direct Answer Results (No CoT)")
    print(f"{'='*60}")
