#!/usr/bin/env python3
"""
处理 ProofWriter 数据用于 CCA 实验（with proof 版本不包含最终答案）

生成 4 个视角：
1. NL with proof: facts + rules + query + NL proof（不含 "The query is true/false"）
2. NL without proof: facts + rules + query
3. Symbolic with proof: facts + rules + query + symbolic proof（不含 "The query is true/false"）
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("用法: python process_for_cca_no_answer.py <input_file> <output_file>")
        print()
        print("示例:")
        print("  python process_for_cca_no_answer.py test_2k_symbolic.jsonl test_2k_for_cca_no_answer.json")
        print()
        print("区别:")
        print("  此版本的 with_proof 不包含最后的 'The query is true/false'")
        print()
