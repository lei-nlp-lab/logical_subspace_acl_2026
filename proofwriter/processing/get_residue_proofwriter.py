...
def main():
    ...
    ap.add_argument("--input", required=True, help="JSON 文件路径（ProntoQA 或 ProofWriter 格式）")
    ...
    ap.add_argument("--view2", default="Symbolic", help="第二个视角的前缀（默认 FOL；ProofWriter 用 Symbolic）")
    args = ap.parse_args()
    ...
    view2_count = count_valid_pairs(data, args.view2, args.max_pairs)
    print(f"Valid pairs: NL={nl_count}, {args.view2}={view2_count}")

    if nl_count == 0 or view2_count == 0:
        raise SystemExit(f"No valid NL/{args.view2} proof pairs found in input.")
    ...
    for rec in pbar:
        ...
        # 提取 NL 和第二个视角的 proof 对
        nl_with, nl_without = get_view_pair(rec, "NL")
        view2_with, view2_without = get_view_pair(rec, args.view2)

        if not (nl_with and nl_without and view2_with and view2_without):
            n_skipped += 1
        ...
        try:
            ...
            # 提取第二个视角的 proof residue
            view2_residue = extract_proof_residue(model, view2_with, view2_without, args.prepend_bos)

            # 保存
            nl_list.append(nl_residue.cpu())      # [L, D]
            view2_list.append(view2_residue.cpu()) # [L, D]
    ...
    VIEW2 = torch.stack(view2_list, dim=0) # [N, L, D]
    emb = torch.stack([NL, VIEW2], dim=0)  # [2, N, L, D]
    ...
    out_obj = {
        ...
        "note": f"tensor[0,i,:,:] = NL CoT residue of pair i; tensor[1,i,:,:] = {args.view2} CoT residue of pair i (averaged over proof tokens only)",
    }
