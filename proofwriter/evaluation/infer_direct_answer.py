...
# -*- coding: utf-8 -*-
"""
Direct Answer baseline for ProofWriter CWA - No CoT, No steering
...
# Direct answer prompt - 不要求 CoT，直接回答 (Binary: True/False)
DIRECT_PROMPT_TEMPLATE = """Given the facts, rules, and query, answer with ONLY one word: True or False.
...
def norm_truth(x: str):
    """ProofWriter CWA: True/False only"""
    if x is None:
        return None
    t = str(x).strip().lower()
...
def main():
    ...
    ap.add_argument("--out_csv", type=str, default="direct_answer_cwa.csv")
    ...
    # 输出结果
    print(f"\n{'='*60}")
    print("Direct Answer Results - ProofWriter CWA (No CoT)")
    print(f"{'='*60}")
    ...
    with open(args.out_csv, "a", newline="", encoding="utf-8") as fcsv:
        ...
        row = [
            ...
            cm["True"]["True"], cm["True"]["False"], cm["True"]["PARSE_FAILED"],
            cm["False"]["True"], cm["False"]["False"], cm["False"]["PARSE_FAILED"],
